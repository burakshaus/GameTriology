...
void *inputThread();
void *gameThread();
void draw_human(human human, obstacle obstacle);
void draw_game_over();
bool collision_detection(human human, obstacle obstacle);
...
// Initialize Human
human init_human() {
    human human = {5, GROUND_LEVEL}; // Start at x=5, y=GROUND_LEVEL
    return human;
}
...
void draw_human(human human, obstacle obstacle) {
    ...
    // Draw Human
    for (int i = 0; i < human.yPos; i++) {
        printf("\n");
    }
    give_space(human.xPos);
    printf(" O\n"); // Head
    give_space(human.xPos);
    printf("/|\\\n"); // Arms and torso
    give_space(human.xPos);
...
// Collision detection
bool collision_detection(human human, obstacle obstacle) {
    if (human.xPos + 5 >= obstacle.xPos && human.yPos == obstacle.yPos) {
...
void *gameThread() {
    ...
    while (1) {
        ...
        // Collision detection
        if (collision_detection(human, obstacle)) {
            draw_game_over();
            break;
        }
        ...
        } else if (human.yPos < GROUND_LEVEL) {
            human.yPos++; // Fall back to ground
        }
